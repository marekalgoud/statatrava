...
export class GraphsComponent implements OnInit {
  ...
  private updateChart(): void {
    ...
    const activitiesByYearsAndMonth = this.activitiesByYearsAndMonth();

    if (activitiesByYearsAndMonth) {
      const root = pack(d3.hierarchy(activitiesByYearsAndMonth as unknown)
        .sum((d: any) => d.value || 0)
      ...
      const zoom = (event: Event, d: any) => {
        ...
        const zoomTo = (v: d3.ZoomView) => {
          ...
          text.style('opacity', d => d.r * k > 20 ? 1 : 0)
            .style('font-size', (d: any) => `${Math.min(2 * d.r * k, (2 * d.r * k - 8) / this.getTextWidth(d.data.name, '10px sans-serif') * 5)}px`);
        }
      }
    }
  }
